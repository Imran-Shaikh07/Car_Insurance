...
const CarDetails = () => {
  ...
  const navLinks = [
    { title: "Overview", icon: <AiOutlineCar /> },
    { title: "Insurance", icon: <AiOutlineSafety /> },
    { title: "Details", icon: <AiOutlineInfoCircle /> },
  ];
        ...
        {isNavOpen && (
          <div className="md:hidden bg-blue-500">
            {navLinks.map((link, index) => (
              <button
                key={index}
                className="block w-full text-white hover:bg-blue-600 px-4 py-2 transition-colors duration-200 flex items-center space-x-2"
                aria-label={link.title}
              >
                {link.icon}
                <span>{link.title}</span>
